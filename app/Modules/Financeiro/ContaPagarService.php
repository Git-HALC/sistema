...
class ContaPagarService
{
    ...
    {
        if (isset($filtros['status']) && $filtros['status'] === 'Vencido') {
    ...
    public function baixar(int $id, array $dados): array
    {
        if (empty($dados['conta_id'])) {
            return ['ok' => false, 'erros' => ['Selecione uma conta bancária.']];
        }
        if (empty($dados['categoria_dre_id'])) {
            return ['ok' => false, 'erros' => ['Selecione uma categoria DRE.']];
        }
    ...
    {
        $erros = [];
        $valor = (float) ($dados['valor'] ?? $dados['valor_original'] ?? 0);
        if ($valor <= 0) {
