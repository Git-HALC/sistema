<div class="container-fluid">
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        ...
        <a href="/sistema_dm/public/admin/financeiro/contas-pagar.php" class="d-none d-sm-inline-block btn btn-sm btn-secondary shadow-sm">
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">Dados da Conta a Pagar</h6>
        </div>
        <div class="card-body">
            <form method="post" action="/sistema_dm/public/admin/financeiro/contas-pagar.php">
                <input type="hidden" name="action" value="<?php echo isset($contaPagar) ? 'atualizar' : 'salvar'; ?>">
                <?php if (isset($contaPagar)): ?>
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-control" id="cliente_id" name="cliente_id" data-placeholder="Digite para buscar por nome, e-mail ou documento">
                            <option value=""></option>
                            <?php if (isset($contaPagar) && !empty($contaPagar['cliente_id'])): 
                                $clienteSel = null;
                                foreach ($clientes as $c) {
                                    if ($c['id'] == $contaPagar['cliente_id']) {
                                        $clienteSel = $c;
                                        break;
                                    }
                                }
                                if ($clienteSel):
                                    ...
                                    $empresaSel = !empty($clienteSel['cpf_cnpj']) ? ' - ' . $clienteSel['cpf_cnpj'] : '';
                        ...
                        <small class="text-muted d-block mt-1">Pesquise fornecedores/clientes existentes ou cadastre um novo sem sair desta tela.</small>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="number" step="0.01" class="form-control" id="valor" name="valor" 
                               value="<?php echo isset($contaPagar) ? ($contaPagar['valor'] ?? ($contaPagar['valor_original'] ?? '')) : ''; ?>" required>
                    </div>
                    
                    <div class="col-md-4 mb-3">
                        <label for="data_vencimento" class="form-label">Data de Vencimento *</label>
                        <input type="date" class="form-control" id="data_vencimento" name="data_vencimento" 
                               value="<?php echo isset($contaPagar) ? $contaPagar['data_vencimento'] : ''; ?>" required>
                    </div>
                </div>
                
                <div class="mb-3">
                    <label for="descricao" class="form-label">Descrição/Histórico</label>
                    <textarea class="form-control" id="descricao" name="descricao" rows="3"><?php echo isset($contaPagar) ? htmlspecialchars($contaPagar['descricao']) : ''; ?></textarea>
                </div>
                
                <div class="mb-3">
                    <label for="observacoes" class="form-label">Observações</label>
                    <textarea class="form-control" id="observacoes" name="observacoes" rows="2"><?php echo isset($contaPagar) ? htmlspecialchars($contaPagar['observacoes']) : ''; ?></textarea>
                </div>
                
                <div class="d-flex justify-content-end">
                    <a href="/sistema_dm/public/admin/financeiro/contas-pagar.php" class="btn btn-secondary me-2">Cancelar</a>
...
$(document).ready(function() {
    ...
    clienteSelectEl.select2({
        ...
        width: '100%',
        ajax: {
            url: '/sistema_dm/public/admin/financeiro/contas-pagar.php?action=buscar-clientes',
            dataType: 'json',
            delay: 300,
    ...
    $('#formNovoCliente').on('submit', function(e) {
        ...
        $.ajax({
            url: '/sistema_dm/public/admin/financeiro/contas-pagar.php?action=cadastrar-cliente',
            method: 'POST',
            data: form.serialize(),
            dataType: 'json'
        }).done(function(resp) {
