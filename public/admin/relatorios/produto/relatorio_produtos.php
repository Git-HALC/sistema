...
use App\Modules\Relatorios\Reports\RelatorioProdutos;
...
$selfUrl       = tenantUrl('admin/relatorios/produto/relatorio_produtos.php');
...
$relatorio = new RelatorioProdutos($db);
...
<div class="container-fluid">
    ...
    .table-warning:hover { background-color: rgba(255,193,7,0.2) !important; }
    [data-theme="dark"] .table-warning { background-color: rgba(255,193,7,0.15) !important; }
    .mobile-card {
    ...
    @media (max-width: 768px) {
        ...
        .filter-section .col-md-3, .filter-section .col-md-4, .filter-section .col-md-5 { flex: 1 1 100%; max-width: 100%; }
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="h3 mb-0">
            <i class="fas fa-boxes me-2"></i>Inventário de Estoque
        </h1>
    ...
    <div class="resumo-card">
        <h4 class="mb-4"><i class="fas fa-chart-bar me-2"></i>Resumo do Inventário</h4>
        <div class="row">
            <div class="col-md-3">
                <div class="resumo-item">
                    <span>Produtos</span>
                    <span class="resumo-value"><?= $resumo['total_produtos'] ?? 0 ?></span>
                </div>
            </div>
            <div class="col-md-3">
                <div class="resumo-item">
                    <span>Ativos</span>
                    <span class="resumo-value positive"><?= $resumo['total_ativos'] ?? 0 ?></span>
                </div>
            </div>
            <div class="col-md-3">
                <div class="resumo-item">
                    <span>Estoque baixo</span>
                    <span class="resumo-value negative"><?= $resumo['estoque_baixo'] ?? 0 ?></span>
                </div>
            </div>
            <div class="col-md-3">
                <div class="resumo-item">
                    <span>Valor em custo</span>
                    <span class="resumo-value">R$ <?= number_format((float)($resumo['valor_custo_total'] ?? 0), 2, ',', '.') ?></span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="card shadow-sm">
        <div class="card-header bg-dark text-white">
            <h5 class="mb-0"><i class="fas fa-list me-2"></i>Posição Atual do Estoque</h5>
        </div>
                ...
                <table class="table table-hover custom-table mb-0">
                    <thead>
                        <tr>
                            ...
                            <th class="text-end">Valor Custo Est.</th>
                            <th class="text-end">Valor Venda Est.</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php if (empty($produtos)): ?>
                            <tr><td colspan="9" class="text-center text-muted py-3">Nenhum produto encontrado.</td></tr>
                        <?php else: ?>
                            <?php foreach ($produtos as $p): ?>
                                <?php $estoqueBaixo = (float)$p['estoque_atual'] <= (float)$p['estoque_minimo']; ?>
                                <tr <?= $estoqueBaixo ? 'class="table-warning"' : '' ?>>
                                    <td><?= htmlspecialchars((string)$p['codigo']) ?></td>
                                    <td><?= htmlspecialchars((string)$p['nome']) ?></td>
                                    <td class="text-center"><?= htmlspecialchars((string)$p['unidade']) ?></td>
                                    <td class="text-end <?= $estoqueBaixo ? 'negative fw-bold' : '' ?>">
                                        <?= number_format((float)$p['estoque_atual'], 2, ',', '.') ?>
                                    </td>
                                    <td class="text-end"><?= number_format((float)$p['estoque_minimo'], 2, ',', '.') ?></td>
                                    <td class="text-end">R$ <?= number_format((float)$p['preco_custo'], 2, ',', '.') ?></td>
                                    <td class="text-end">R$ <?= number_format((float)$p['preco_venda'], 2, ',', '.') ?></td>
                                    <td class="text-end">R$ <?= number_format((float)$p['valor_custo_estoque'], 2, ',', '.') ?></td>
                                    <td class="text-end">R$ <?= number_format((float)$p['valor_venda_estoque'], 2, ',', '.') ?></td>
                                </tr>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </tbody>
        ...
        <?php foreach ($produtos as $p): ?>
            ...
            <div class="mobile-card">
                <div class="mobile-card-header">
                    <div>
                        <strong><?= htmlspecialchars((string)$p['nome']) ?></strong>
                        <small class="text-muted ms-2"><?= htmlspecialchars((string)$p['codigo']) ?></small>
                    </div>
                    <?= $estoqueBaixo ? '<span class="badge bg-warning text-dark">Estoque baixo</span>' : '' ?>
                </div>
                <div class="mobile-card-body">
                    <div class="mobile-card-row">
                        <span class="mobile-card-label">Unidade</span>
                        <span class="mobile-card-value"><?= htmlspecialchars((string)$p['unidade']) ?></span>
                    </div>
                    <div class="mobile-card-row">
                        <span class="mobile-card-label">Estoque / Mínimo</span>
                        <span class="mobile-card-value <?= $estoqueBaixo ? 'negative' : '' ?>">
                            <?= number_format((float)$p['estoque_atual'], 2, ',', '.') ?> / <?= number_format((float)$p['estoque_minimo'], 2, ',', '.') ?>
                        </span>
                    </div>
                    <div class="mobile-card-row">
                        <span class="mobile-card-label">Preço Custo</span>
                        <span class="mobile-card-value">R$ <?= number_format((float)$p['preco_custo'], 2, ',', '.') ?></span>
                    </div>
                    <div class="mobile-card-row">
                        <span class="mobile-card-label">Preço Venda</span>
                        <span class="mobile-card-value">R$ <?= number_format((float)$p['preco_venda'], 2, ',', '.') ?></span>
                    </div>
                </div>
            </div>
        <?php endforeach; ?>
