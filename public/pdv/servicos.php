...
if (in_array($action, ['novo', 'salvar'], true)) {
    $destino = tenantCleanUrl('pdv/servicos') . ($action !== '' ? '?action=' . urlencode($action) : '');
    $caixaId = $lancamentoController->resolverCaixaOuRenderizarSelecao($destino, 'servicos');
    if ($caixaId !== null) {
...
$GLOBALS['__dm_pdv_active'] = 'servicos';
...
require __DIR__ . '/../admin/servicos.php';
